...
describe('Tests des routes DetailActiviteSportive', () => {
    it('GET /api/detailactivitesportive doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/detailactivitesportive');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/detailactivitesportive avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/detailactivitesportive')
