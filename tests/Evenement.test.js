...
describe('Tests des routes Evenement', () => {
    it('GET /api/evenement doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/evenement');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/evenement avec données invalides doit retourner un statut 400', async () => {
