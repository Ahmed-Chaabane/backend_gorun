...
describe('Tests des routes DefiParticipants', () => {
    it('GET /api/defiparticipants doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/defiparticipants');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/defiparticipants avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/defiparticipants')
