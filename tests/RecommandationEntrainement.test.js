...
describe('Tests des routes RecommandationEntrainement', () => {
    it('GET /api/recommandationentrainement doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/recommandationentrainement');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/recommandationentrainement avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/recommandationentrainement')
