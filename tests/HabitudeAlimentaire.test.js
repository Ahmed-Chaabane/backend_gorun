...
describe('Tests des routes HabitudeAlimentaire', () => {
    it('GET /api/habitudealimentaire doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/habitudealimentaire');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/habitudealimentaire avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/habitudealimentaire')
