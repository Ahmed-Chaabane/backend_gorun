...
describe('Tests des routes DefiCommunautaire', () => {
    it('GET /api/deficommunautaire doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/deficommunautaire');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/deficommunautaire avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/deficommunautaire')
