...
describe('Tests des routes RecuperationBlessure', () => {
    it('GET /api/recuperationblessure doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/recuperationblessure');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/recuperationblessure avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/recuperationblessure')
