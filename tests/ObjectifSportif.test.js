...
describe('Tests des routes ObjectifSportif', () => {
    it('GET /api/objectifsportif doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/objectifsportif');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/objectifsportif avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/objectifsportif')
