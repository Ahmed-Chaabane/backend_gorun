...
describe('Tests des routes DefiProgres', () => {
    it('GET /api/defiprogres doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/defiprogres');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/defiprogres avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/defiprogres')
