...
describe('Tests des routes Aliments', () => {
    it('GET /api/aliments doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/aliments');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/aliments avec données invalides doit retourner un statut 400', async () => {
