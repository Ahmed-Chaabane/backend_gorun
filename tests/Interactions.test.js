...
describe('Tests des routes Interactions', () => {
    it('GET /api/interactions doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/interactions');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/interactions avec données invalides doit retourner un statut 400', async () => {
