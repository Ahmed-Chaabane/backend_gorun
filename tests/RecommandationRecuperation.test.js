...
describe('Tests des routes RecommandationRecuperation', () => {
    it('GET /api/recommandationrecuperation doit retourner un statut 200', async () => {
        const response = await request(app).get('/api/recommandationrecuperation');
        expect(response.status).toBe(200); // Vérifie que le statut est 200
    });

    it('POST /api/recommandationrecuperation avec données invalides doit retourner un statut 400', async () => {
        const response = await request(app)
            .post('/api/recommandationrecuperation')
